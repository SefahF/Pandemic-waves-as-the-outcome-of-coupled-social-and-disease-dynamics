...
Threshold_osi = [0.10, 0.10 + 0.25, 0.10,0.10+0.25,0.10,0.10,0.10,0.25 + 0.1,0.10,0.20,0.20+0.15,0.30,0.10,0.10,0.10+0.25,0.10+0.15,0.20,0.10+0.15,0.10]
x0_interval = [(0.0,0.20), (0.0,0.050), (0.0,0.20),(0.0,0.10),(0.0,0.25),(0.0,0.20),(0.0,0.10),(0.0,0.050),(0.0,0.10), (0.0,0.10),(0.0,0.10),(0.0,0.20),(0.0,0.10),(0.0,0.20),(0.0,0.10),(0.0,0.20),(0.0,0.20),(0.0,0.10),(0.0,0.10)]
Peak_interval = [(22, 36), (53, 67), (63, 77), (108, 122), (109, 123), (28, 42), (57, 71), (61, 75), (71, 85), (53, 67), (32, 46), (33, 47), (34, 48), (23, 37), (32, 46), (37, 51), (42, 56), (74, 88), (19, 33)]
...
cutoff = Cutoff[idx]
peak_interval = Peak_interval[idx] 
slope = Slope[idx]
...
logging.basicConfig(filename = "example_osi.log", encoding ='utf-8', level = logging.INFO)
...
def error(sim,x0,x1):
    maxs = find_peaks(sim[2:], height = [100*(1/pop),1])[0]
    err = mean(square(data - sim[int(t_data[0]):]))
    return err


def error_x(X):
    err = mean(square(stringency_data[:int(t_data[-1])] - X[int(t_data[0]):]))
    return err
...
def odex(params, variant, t_end): # returns simulation data for given params, ICs, and duration
	...
	if variant == 3:
		def fun(t,y):
			...
			e2 = gamma*y[1]
			e3 = kappa*y[2]*(1-y[2])*((1/(eta_0))*y[1]*exp(-lamb*t) - c + sigma*(2*y[2] -1) )
...
def area_between_curves(data, model): # Get x and y values from both curves
    ...
    area = np.trapz(np.abs(y1_interp - y2_interp), x_values) # Calculate the area using the trapezoidal rule
    return area
...
def rejection(N,filename, errAcpt, errAcptx): # rejection abc with uniform priors
	...
	while successes < N:
		...
		XX, sim = odex(particle, variant, t_data[-1]) # simulate the random parameters
		negative_values = [neg_val for neg_val in sim if neg_val < 0]
		if len(negative_values) == 0 and len(sim) == t_data[-1] + 1 and error(sim,x0,x1)< errAcpt and error_x(XX) < errAcptx:
			trials.append((error(sim,x0,x1), particle[mask], error_x(XX))) # store in tuple : error, fitted params
			successes += 1
...
def smcSample(N, nParams, nSamples, samples, weights,filename, errAcpt, errAcptx): # takes samples from previous population and generates new population
	...
	mask = isnan(params)
	# ranges and bounds define permissible random param values
	ranges = array([r_alpha[1] - r_alpha[0], r_beta[1] - r_beta[0], r_b[1] - r_b[0], r_gamma[1] - r_gamma[0], r_c[1] - r_c[0], r_eta_0[1] - r_eta_0[0],
				  r_epsilon[1] - r_epsilon[0], r_eta_1[1] - r_eta_1[0], r_kappa[1] - r_kappa[0], r_lamb[1] - r_lamb[0],
				  r_sigma[1] - r_sigma[0], r_phi[1] - r_phi[0], r_E0[1] - r_E0[0], r_I0[1] - r_I0[0], r_x0[1] - r_x0[0], r_Ii0[1] - r_Ii0[0]])[mask]
	...
	print ("Acceptance Rate: ", successes/(fails+successes), fails, "fails and", successes, "successes in", str(datetime.timedelta(seconds=(time.time()-ti))))
	sortedTrials = sorted(trials, key=lambda x:x[0]) # sort particles
	print("best error:", (sortedTrials[0])[0] , (sortedTrials[2])[0])
	return sortedTrials
...
def smc(N,filename, errAcpt, errAcptx): # generates list of particles from successive iterations of smcSample()
	...
	for pop in range(nPop):
		print("Next iteration", pop)
		error_sample = [samples[i][0] for i in range(nSamples)]
		new_errAcpt = np.max(error_sample)
		if new_errAcpt < errAcpt:
			errAcpt = new_errAcpt
		else:
			errAcpt = errAcpt
		errAcptx = abs(errAcptx - 0.005)
	...
	ErrorX = [best[i][2] for i in range(len(sorted_particles))]
	parameterset = pd.DataFrame(parameters,columns=[ 'beta', 'b', 'gamma', 'c', 'eta_0', 'epsilon', 'kappa', 'lambda', 'phi', 'I0', 'x0','C0'])
	parameterset["Error_I"] = Error
	parameterset["Error_X"] = ErrorX
	parameterset.to_csv(filename[len(Countries):-4] + "_" + str(n) + "_" + "sirx_new.csv",index=False)
	...
	for trial in trials:
		...
		for i in range(len(particle)):
			if isnan(particle[i]):
				...
				counter += 1
		trial_data = odex(particle, variant, t_end)
...
params_v3     = array([       0,     nan,     nan,     nan,     nan,     nan,     nan,     0,     nan,     nan,     0,     nan,     0,     nan,     nan,     nan])
...
printEvery = 5 # print progress every printEvery simulation runs
bestN = 10 # plot the bestN best particles at the end


#------ ORIGINAL RANGE -------# job array with index values 0, 1, 2, 3, 4
...
for h in range(10):
	errAcptx = Threshold_osi[idx] + 0.5
	n = h
